...
function get_elevelddata()
    ...
    for studynbr = 1:nstudy
        ...
        for id = firstid:lastid
            ...
            θeleveld,infusionrate, bolusdose, t, hs, youts = get_patientdata(id, modeldf, inputdf) # get patient data
            ...
            covariates = Float32.([continuous_inputs[1] / input_normalization[1], continuous_inputs[2] / input_normalization[2], continuous_inputs[3] / input_normalization[3], m1f2 - 1.5f0, a1v2 - 1.5f0]) # scaled input variables, AGE (0->1), WGT (0->1), GDR (male -0.5, female 0.5), AV (arterial -0.5, venous 0.5)
